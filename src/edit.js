  ...
  <Flex align="center" style={{ marginBottom: "12px" }}>
    ...
    <FlexItem>
      <Dropdown
        ...
        renderContent={() => (
          <ColorPicker color={color} onChange={onChange} enableAlpha={true} />
        )}
      />
    </FlexItem>
  </Flex>
...
export default function Edit({ attributes, setAttributes }) {
  ...
  const getGroupInfo = (group) => {
    if (!group || group.length === 0)
      return { source: null, targets: [], isDiagonal: false };
    const hasDiagonal = group[group.length - 1] === "d";
    const numericEntries = group.filter((e) => typeof e === "number");
    return {
      source: numericEntries[0] || null,
  ...
  return (
    <>
      <InspectorControls>
        ...
        {/* Grid Size Control */}
        <PanelBody
          title={__("Grid Size", "icon-grid-unlimited")}
          initialOpen={true}>
          <Flex gap={4}>
          ...
          <p
            ...
            {gridRows} × {gridCols} = {totalTilesInGrid}{" "}
            {__("tiles", "icon-grid-unlimited")}
          </p>
          ...
          <TextControl
            ...
            help={__(
              "Enter a value with unit (e.g., 0.5rem, 1rem, 10px)",
              "icon-grid-unlimited",
            )}
          />
          ...
          <ToggleControl
            ...
            help={__(
              "Show a subgrid initially, expand to full grid on trigger",
              "icon-grid-unlimited",
            )}
          />
          ...
          {enlargeEnabled && (
            <>
              ...
              <Flex gap={4}>
                <FlexItem style={{ flex: 1 }}>
                  <RangeControl
                    ...
                    onChange={(v) =>
                      setAttributes({
                        subgridRows: Math.min(v, gridRows - subgridStartRow),
                      })
                    }
                    min={1}
                    max={gridRows}
                ...
                <FlexItem style={{ flex: 1 }}>
                  <RangeControl
                    ...
                    onChange={(v) =>
                      setAttributes({
                        subgridCols: Math.min(v, gridCols - subgridStartCol),
                      })
                    }
                    min={1}
                    max={gridCols}
                ...
                {Array.from({ length: gridRows * gridCols }, (_, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={
                        isInSubgrid ? "Subgrid (visible)" : "Expansion (hidden)"
                      }
                    />
                  );
                })}
              ...
              <p
                ...
                {__(
                  "Blue = Subgrid (visible), Grey = Expansion tiles",
                  "icon-grid-unlimited",
                )}
              </p>
              ...
              <TextControl
                ...
                help={__(
                  "CSS selector for button that triggers expansion",
                  "icon-grid-unlimited",
                )}
              />
              <TextControl
              ...
              <RangeControl
                ...
                step={50}
                help={__(
                  "Expand when scrolled past this position (0 = disabled)",
                  "icon-grid-unlimited",
                )}
              />
            </>
        ...
        {/* Timing Settings */}
        <PanelBody
          title={__("Timing Settings", "icon-grid-unlimited")}
          initialOpen={false}>
          <RangeControl
          ...
          <RangeControl
            ...
            step={50}
            help={__(
              "Negative = next round starts early (overlap), Positive = delay between rounds",
              "icon-grid-unlimited",
            )}
          />
          <RangeControl
            ...
            step={50}
            help={__(
              "Delay after entrance animation before connection rounds start",
              "icon-grid-unlimited",
            )}
          />
        </PanelBody>
        ...
        {/* Line Settings */}
        <PanelBody
          title={__("Line Settings", "icon-grid-unlimited")}
          initialOpen={false}>
          <PopupColorPicker
        ...
        {/* Icon Style Settings */}
        <PanelBody
          title={__("Icon Style", "icon-grid-unlimited")}
          initialOpen={false}>
          <RangeControl
          ...
          <ToggleControl
            ...
            help={__(
              "When disabled, stroke width scales with the icon (sharper when scaled down)",
              "icon-grid-unlimited",
            )}
          />
          <SelectControl
            ...
            options={[
              {
                label: __("Auto (default)", "icon-grid-unlimited"),
                value: "auto",
              },
              {
                label: __("Crisp Edges (sharp lines)", "icon-grid-unlimited"),
                value: "crispEdges",
              },
              {
                label: __("Geometric Precision", "icon-grid-unlimited"),
                value: "geometricPrecision",
              },
              {
                label: __("Optimize Speed", "icon-grid-unlimited"),
                value: "optimizeSpeed",
              },
            ]}
            help={__(
              'Try "Crisp Edges" for sharper strokes like CSS borders',
              "icon-grid-unlimited",
            )}
          />
        </PanelBody>
        ...
        {/* Hover Settings */}
        <PanelBody
          title={__("Hover Settings", "icon-grid-unlimited")}
          initialOpen={false}>
          <RangeControl
          ...
          <ToggleControl
            ...
            help={__(
              "Animate only shadow opacity for smoother transitions",
              "icon-grid-unlimited",
            )}
          />
        </PanelBody>
        ...
        {/* Layout Settings */}
        <PanelBody
          title={__("Layout Settings", "icon-grid-unlimited")}
          initialOpen={false}>
          <ToggleControl
            ...
            help={__(
              "Makes the grid stick to viewport while scrolling (for column layouts)",
              "icon-grid-unlimited",
            )}
          />
          {stickyEnabled && (
          ...
          <ToggleControl
            ...
            help={__(
              "Dynamically calculates top offset to center grid in viewport",
              "icon-grid-unlimited",
            )}
          />
          {attributes.centerVertically && (
            <TextControl
              ...
              help={__(
                "ID or class of element to apply centering to. Leave empty for grid itself.",
                "icon-grid-unlimited",
              )}
            />
          )}
        ...
        {/* Mobile Mode */}
        <PanelBody
          title={__("Mobile Mode", "icon-grid-unlimited")}
          initialOpen={false}>
          <ToggleControl
            label={__("Enable Mobile Mode", "icon-grid-unlimited")}
            checked={attributes.mobileEnabled}
            onChange={(v) => setAttributes({ mobileEnabled: v })}
            help={__(
              "Compact grid layout for small screens: no animation rounds, packed tiles, random highlight loop",
              "icon-grid-unlimited",
            )}
          />
          {attributes.mobileEnabled && (
            <>
              <RangeControl
                label={__("Breakpoint (px)", "icon-grid-unlimited")}
                value={attributes.mobileBreakpoint || 768}
              ...
              />
              <RangeControl
                label={__("Mobile Columns", "icon-grid-unlimited")}
                value={attributes.mobileCols || 3}
                onChange={(v) => setAttributes({ mobileCols: v })}
                min={1}
                max={6}
                step={1}
                help={__(
                  "Number of columns in the compact mobile grid",
                  "icon-grid-unlimited",
                )}
              />
              <RangeControl
                label={__("Highlight Duration (ms)", "icon-grid-unlimited")}
                value={attributes.mobileHighlightDuration || 2000}
                onChange={(v) => setAttributes({ mobileHighlightDuration: v })}
                min={500}
                max={8000}
                step={100}
                help={__(
                  "How long each tile stays highlighted",
                  "icon-grid-unlimited",
                )}
              />
              <RangeControl
                label={__("New Tile Interval (ms)", "icon-grid-unlimited")}
                value={attributes.mobileHighlightInterval || 800}
                onChange={(v) => setAttributes({ mobileHighlightInterval: v })}
                min={200}
                max={4000}
                step={100}
                help={__(
                  "Time between activating new random tiles (3 always active)",
                  "icon-grid-unlimited",
                )}
              />
            </>
        ...
        {/* Animation Rounds */}
        <PanelBody
          title={__("Animation Rounds", "icon-grid-unlimited")}
          initialOpen={false}>
          <p className="components-base-control__help">
            {__(
              "Define connection patterns for the animation loop.",
              "icon-grid-unlimited",
            )}
          </p>
          <Button variant="secondary" onClick={() => setRoundsModalOpen(true)}>
            {__("Edit Animation Rounds", "icon-grid-unlimited")}
          </Button>
          <p style={{ marginTop: "10px", fontSize: "12px", color: "#666" }}>
            {animationRounds.length}{" "}
            {__("rounds defined", "icon-grid-unlimited")}
          </p>
        </PanelBody>

        {/* Grid Editor */}
        <PanelBody
          title={__("Grid & Icons", "icon-grid-unlimited")}
          initialOpen={false}>
          <p className="components-base-control__help">
            {__(
              "Configure labels, SVGs, and SEO data for each tile.",
              "icon-grid-unlimited",
            )}
          </p>
          <Button variant="secondary" onClick={() => setGridModalOpen(true)}>
            {__("Open Grid Editor", "icon-grid-unlimited")}
          </Button>
          <p style={{ marginTop: "10px", fontSize: "12px", color: "#666" }}>
            {iconLabels.filter((l) => l !== "").length}{" "}
            {__("tiles configured", "icon-grid-unlimited")}
          </p>
        </PanelBody>
      </InspectorControls>
          ...
          <Flex style={{ gap: "20px", minHeight: "500px" }}>
                    ...
                    style={{
                      ...
                      background:
                        selectedRoundIndex === roundIndex
                          ...
                          : "#f0f0f0",
                      color:
                        selectedRoundIndex === roundIndex ? "#fff" : "#333",
                      borderRadius: "4px",
                      cursor: "pointer",
                      fontSize: "13px",
                    }}
                    ...
                    <span style={{ flex: 1 }}>
                      ...
                      <span
                        style={{
                          fontSize: "11px",
                          opacity:
                            selectedRoundIndex === roundIndex ? 0.8 : 0.6,
                          marginLeft: "4px",
                        }}>
                        ({round.length}{" "}
                        {round.length === 1 ? "group" : "groups"})
                      </span>
                    </span>
                    <Button
                      isSmall
            ...
            <FlexItem style={{ flex: "1 1 auto", position: "relative" }}>
              <div style={{ marginBottom: "10px" }}>
                <p style={{ fontWeight: "600", marginBottom: "5px" }}>
                  {connectionMode
                    ? __(
                        "Click tiles to create connection:",
                        "icon-grid-unlimited",
                      )
                ...
                {connectionMode && (
                  <p
                    ...
                    {pendingConnection.source === null
                      ? __(
                          "Click a tile to set as SOURCE",
                          "icon-grid-unlimited",
                        )
                      : __(
                          "Click tiles to add/remove TARGETS. Click source again to finish.",
                          "icon-grid-unlimited",
                        )}
                  </p>
                )}
              ...
              <div
                ...
                {Array.from(
                  ...
                  (_, displayIndex) => {
                    ...
                    const hasContent =
                      iconLabels[storageIndex] || iconSvgs[storageIndex];

                    // Check if this tile is part of selected group
                    const currentGroup =
                      selectedRoundIndex !== null && selectedGroupIndex !== null
                        ? animationRounds[selectedRoundIndex]?.[
                            selectedGroupIndex
                          ]
                        : null;
                    const groupInfo = currentGroup
                      ? getGroupInfo(currentGroup)
                      : { source: null, targets: [] };
                    ...
                    if (
                      ...
                      animationRounds[selectedRoundIndex]?.forEach((g) => {
                        ...
                        if (
                          gi.source === tileNumber ||
                          gi.targets.includes(tileNumber)
                        ) {
                          isInRound = true;
                        }
                      });
                    ...
                    const tileSize = Math.max(
                      28,
                      Math.min(42, 400 / Math.max(gridRows, gridCols)),
                    );
                    ...
                    return (
                      <div
                        ...
                        title={`${
                          iconLabels[storageIndex] || "Tile"
                        } ${tileNumber}${isSource ? " (Source)" : ""}${
                          isTarget ? " (Target)" : ""
                        }`}>
                        {tileNumber}
                      </div>
            ...
            <FlexItem
              ...
              {selectedRoundIndex !== null ? (
                <>
                  <p style={{ fontWeight: "600", marginBottom: "10px" }}>
                    {__("Groups in Round", "icon-grid-unlimited")}{" "}
                  ...
                  <div
                    ...
                    {(animationRounds[selectedRoundIndex] || []).map(
                      (group, groupIndex) => {
                        ...
                        return (
                          <div
                            ...
                            <Flex
                              ...
                              <FlexItem>
                                <Button
                                  ...
                                  variant="tertiary"
                                  onClick={() =>
                                    editGroup(selectedRoundIndex, groupIndex)
                                  }
                                  style={{
                                    padding: "2px 6px",
                                    fontSize: "11px",
                                ...
                                <Button
                                  ...
                                  variant="tertiary"
                                  onClick={() =>
                                    removeGroup(selectedRoundIndex, groupIndex)
                                  }
                                  style={{
                                    padding: "2px 6px",
                                    fontSize: "11px",
                            ...
                            <div style={{ fontSize: "11px", color: "#555" }}>
                              <div>
                                <strong>
                                  {__("Source:", "icon-grid-unlimited")}
                                </strong>{" "}
                                {info.source || "—"}
                              </div>
                              <div>
                                <strong>
                                  {__("Targets:", "icon-grid-unlimited")}
                                </strong>{" "}
                            ...
                            <ToggleControl
                              ...
                              onChange={() =>
                                toggleDiagonal(selectedRoundIndex, groupIndex)
                              }
                ...
                <div
                  ...
                  <p>
                    {__(
                      "← Select a round to edit its groups",
                      "icon-grid-unlimited",
                    )}
                  </p>
                </div>
        ...
        <Modal
          ...
          <Flex>
            ...
            <FlexItem style={{ flex: "0 0 auto" }}>
              <p>
                <strong>
                  {__("Click a tile to edit:", "icon-grid-unlimited")}
                </strong>
              </p>
              <div
                ...
                {Array.from({ length: 144 }, (_, storageIndex) => {
                  ...
                  const displayNumber = isInBounds
                    ? row * gridCols + col + 1
                    : "";
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background:
                          dragFromTile !== null && dragFromTile !== storageIndex
                      ...
                      title={`${hasContent ? "Drag to move • " : ""}Storage ${
                        storageIndex + 1
                      } (Row ${row + 1}, Col ${col + 1})${
                        isInBounds ? "" : " - outside grid"
                      }${label ? ": " + label : ""}`}>
                      {displayNumber || "·"}
                    </div>
                  );
              ...
              <p style={{ fontSize: "10px", color: "#666", marginTop: "5px" }}>
                {__(
                  "Drag tiles to move settings • Greyed tiles are outside current grid",
                  "icon-grid-unlimited",
                )}
              </p>
            </FlexItem>
              ...
              {selectedTile !== null ? (
                (() => {
                  ...
                  return (
                    <div>
                      <h3>
                        {__("Tile", "icon-grid-unlimited")}{" "}
                        {displayNumber || `(${row + 1}-${col + 1})`}
                        <span
                          style={{
                            fontSize: "12px",
                            fontWeight: "normal",
                      ...
                      {!isInBounds && (
                        <div
                          ...
                          ⚠️{" "}
                          {__(
                            "This tile is outside the current grid bounds. Data is preserved and will be visible if you increase grid size.",
                            "icon-grid-unlimited",
                          )}
                        </div>
                      )}
                      ...
                      <TextareaControl
                        ...
                        placeholder={__(
                          'Paste SVG path here (e.g., <polygon points="..."/>)',
                          "icon-grid-unlimited",
                        )}
                        rows={4}
                      ...
                      <div
                        ...
                        <ToggleControl
                          label={__(
                            "Enable Block (replaces SVG)",
                            "icon-grid-unlimited",
                          )}
                          checked={getTileBlockSettings(selectedTile).enabled}
                          onChange={(v) =>
                            updateTileBlockSettings(selectedTile, "enabled", v)
                          }
                          help={__(
                            "When enabled, a Gutenberg block is used instead of the SVG icon.",
                            "icon-grid-unlimited",
                          )}
                        />
                        {getTileBlockSettings(selectedTile).enabled && (
                          <>
                            <Flex gap={2} style={{ marginBottom: "12px" }}>
                              <FlexItem>
                                <Button
                                  variant="primary"
                                  onClick={() =>
                                    setBlockEditorTile(selectedTile)
                                  }>
                                  {tileBlocks[selectedTile]
                                    ? __("Edit Block", "icon-grid-unlimited")
                                    : __("Add Block", "icon-grid-unlimited")}
                                </Button>
                              </FlexItem>
                              {tileBlocks[selectedTile] && (
                                <FlexItem>
                            ...
                            <ToggleControl
                              ...
                              checked={
                                getTileBlockSettings(selectedTile).hoverOpacity
                              }
                              onChange={(v) =>
                                updateTileBlockSettings(
                                  selectedTile,
                                  "hoverOpacity",
                                  v,
                                )
                            ...
                            <ToggleControl
                              label={__(
                                "Scale + shadow (like SVG tiles)",
                                "icon-grid-unlimited",
                              )}
                              checked={
                                getTileBlockSettings(selectedTile).hoverScale
                              }
                              onChange={(v) =>
                                updateTileBlockSettings(
                                  selectedTile,
                            ...
                            <h4
                              ...
                              {__("Display Options", "icon-grid-unlimited")}
                            </h4>
                            <ToggleControl
                              label={__("Show label", "icon-grid-unlimited")}
                              checked={
                                getTileBlockSettings(selectedTile).showLabel
                              }
                              onChange={(v) =>
                                updateTileBlockSettings(
                                  selectedTile,
                      ...
                      <div
                        ...
                        <ToggleControl
                          label={__(
                            "Custom Icon Settings for this tile",
                            "icon-grid-unlimited",
                          )}
                          checked={
                            perTileIconSettings[selectedTile]?.enabled || false
                          }
                          onChange={(v) =>
                            updatePerTileSettings(selectedTile, "enabled", v)
                          }
                        />
                        {perTileIconSettings[selectedTile]?.enabled && (
                          <>
                            ...
                            <Flex gap={3} style={{ marginBottom: "8px" }}>
                              <FlexItem style={{ flex: 1 }}>
                                <RangeControl
                                  ...
                                  value={
                                    perTileIconSettings[selectedTile]
                                      ?.offsetX || 0
                                  }
                                  onChange={(v) =>
                                    updatePerTileSettings(
                                      selectedTile,
                                      "offsetX",
                                      v,
                                    )
                            ...
                            <RangeControl
                              label={__("Icon Scale", "icon-grid-unlimited")}
                              value={
                                perTileIconSettings[selectedTile]?.scale || 1
                              }
                              onChange={(v) =>
                                updatePerTileSettings(selectedTile, "scale", v)
                              }
                              min={0.5}
                              max={2}
                              step={0.05}
                            ...
                            <hr
                              style={{ margin: "12px 0", borderColor: "#ddd" }}
                            />
                            <p
                              style={{
                                fontWeight: "500",
                            ...
                            <ToggleControl
                              label={__(
                                "Center Enlarged Cell",
                                "icon-grid-unlimited",
                              )}
                              checked={
                                perTileIconSettings[selectedTile]?.centerCell ||
                                false
                              }
                              onChange={(v) =>
                                updatePerTileSettings(
                            ...
                            <ToggleControl
                              label={__(
                                "Scale Label with Cell",
                                "icon-grid-unlimited",
                              )}
                              checked={
                                perTileIconSettings[selectedTile]?.scaleLabel ||
                                false
                              }
                              onChange={(v) =>
                                updatePerTileSettings(
                      ...
                      {iconLabels[selectedTile] && (
                        <>
                          <h4 style={{ marginTop: "20px" }}>
                            {__("SEO Data", "icon-grid-unlimited")}
                          </h4>
                          <TextControl
                            label={__("URL", "icon-grid-unlimited")}
                            value={seoData[iconLabels[selectedTile]]?.url || ""}
                            onChange={(v) =>
                              updateSeoData(iconLabels[selectedTile], "url", v)
                            }
                          ...
                          <TextControl
                            ...
                            value={
                              seoData[iconLabels[selectedTile]]?.description ||
                              ""
                            }
                            onChange={(v) =>
                              updateSeoData(
                            ...
                            placeholder={__(
                              "SEO description for this service",
                              "icon-grid-unlimited",
                            )}
                          />
                          <TextControl
                            ...
                            value={
                              seoData[iconLabels[selectedTile]]?.serviceType ||
                              ""
                            }
                            onChange={(v) =>
                              updateSeoData(
                      ...
                      <div
                        ...
                        <ToggleControl
                          ...
                          help={__(
                            "Enable this tile as a source for WP Logo Explode transitions",
                            "icon-grid-unlimited",
                          )}
                        />
                        {perTileIconSettings[selectedTile]
                          ...
                          <>
                            <TextControl
                              ...
                              help={__(
                                "Unique identifier for this transition source",
                                "icon-grid-unlimited",
                              )}
                            />
                            <RangeControl
                            ...
                            <PopupColorPicker
                              label={__(
                                "Explode Background Override",
                                "icon-grid-unlimited",
                              )}
                              color={
                                perTileIconSettings[selectedTile]
                                  ?.transitionColorOverride || ""
                              }
                              onChange={(color) =>
                                updatePerTileSettings(
                                  selectedTile,
      ...
      {blockEditorTile !== null && (
        <Modal
          title={
            __("Edit Block Content", "icon-grid-unlimited") +
            ` - Tile ${blockEditorTile + 1}`
          }
          ...
          <div style={{ marginBottom: "15px" }}>
            <p style={{ color: "#666", fontSize: "13px" }}>
              {__(
                "Add blocks below. They will be rendered inside the tile on the frontend.",
                "icon-grid-unlimited",
              )}
            </p>
          </div>
          ...
          <div
            ...
            <TextareaControl
              ...
              placeholder={__(
                'Paste block HTML, shortcodes, or any HTML content here.\n\nExamples:\n• [my_shortcode]\n• <div class="my-block">Content</div>\n• <!-- wp:image {"id":123} --><figure>...</figure><!-- /wp:image -->',
                "icon-grid-unlimited",
              )}
              rows={12}
      ...
      <div {...blockProps}>
        <div className="icon-grid-editor-preview">
          <div
            className="icon-grid-preview-grid"
            style={{
              display: "grid",
              gridTemplateColumns: `repeat(${gridCols}, 1fr)`,
          ...
          <p
            ...
            {gridRows}×{gridCols}{" "}
            {__(
              "Icon Grid Preview - Animation plays on frontend",
              "icon-grid-unlimited",
            )}
          </p>
        </div>
